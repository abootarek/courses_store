...
class _EditCourseScreenState extends State<EditCourseScreen> {
  ...
  void initState() {
    ...
    _discountController = TextEditingController(text: widget.course.discount.toString());
    _videoUrlController = TextEditingController(text: widget.course.videoUrl);
    
    // Load existing image
    if (widget.course.imageBase64.isNotEmpty) {
      _imageBase64 = widget.course.imageBase64;
      try {
        _imageBytes = base64Decode(widget.course.imageBase64);
      } catch (e) {
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: isLoading
                          ...
                          : () {
                              if (_formKey.currentState!.validate()) {
                                ...
                                context.read<AdminCubit>().updateCourse(updatedCourse);
                              }
                            },
                      ...
                      child: isLoading
                          ...
                          : const Text('Update Course', style: TextStyle(fontSize: 16)),
                    ),
                    const SizedBox(height: 16),
                    OutlinedButton.icon(
                      onPressed: () {
                        context.push('/manage-lessons', extra: widget.course);
                      },
                      icon: const Icon(Icons.list),
                      label: Text('Manage Lessons (${widget.course.lessonsCount})'),
                      style: OutlinedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(vertical: 16),
                      ),
                    ),
